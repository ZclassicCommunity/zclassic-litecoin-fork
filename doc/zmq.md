...
The Zclassic Core daemon can be configured to act as a trusted "border
router", implementing the zclassic wire protocol and relay, making
...
The ZeroMQ feature in Zclassic Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building zclassicd:
...
For instance:
    ...
    $ zclassicd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/zclassicd.tx.raw \
...
From the perspective of zclassicd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into zclassicd directly. Furthermore, no information is
...
using. Zclassicd appends an up-counting sequence number to each
