...
def download_binary(tag, args) -> int:
    if Path(tag).is_dir():
        ...
        shutil.rmtree(tag)
    Path(tag).mkdir()
    release_path = 'zclassic-{}'.format(tag[1:])
    os = 'linux' # TODO
    tarball = 'zclassic-{tag}-{platform}.tar.gz'.format(
        tag=tag[1:], platform=args.platform)
    tarballUrl = 'https://download.zclassic.org/{release_path}/{os}/{tarball}'.format(
    ...
    ret = subprocess.run(['tar', '-zxf', tarball, '-C', tag,
                          '--strip-components=1',
                          'zclassic-{tag}'.format(tag=tag[1:])]).returncode
    if ret:
        return ret
...
def build_release(tag, args) -> int:
    githubUrl = "https://github.com/zclassic-project/zclassic"
    if args.remove_dir:
        if Path(tag).is_dir():
            shutil.rmtree(tag)
    ...
    with pushd(tag):
        ...
        files = ['zclassicd', 'zclassic-cli', 'zclassic-tx']
        for f in files:
