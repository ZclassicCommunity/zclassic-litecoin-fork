#!/usr/bin/env python3
# Copyright (c) 2014-2022 The Zclassic Core developers
...
def create_non_hd_wallet(chain, options):    
    ...
    conf_file = os.path.join(data_dir, 'zclassic.conf')
    ...
    v15_node = TestNode(
        ...
        timewait=60,
        timeout_factor=1.0,
        bitcoind=os.path.join(bin_dir, 'zclassicd'),
        bitcoin_cli=os.path.join(bin_dir, 'zclassic-cli'),
        version=version,
        coverage_dir=None,
        cwd=options.tmpdir,
        extra_args=["-usehd=0"],
