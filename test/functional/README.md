...
with the zclassicd(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a zclassicd.  `P2PInterface`
contains the higher level logic for processing P2P payloads and connecting to
the Zclassic Core node application logic. For custom behaviour, subclass the
...
Test objects for interacting with a zclassicd node over the p2p interface.
