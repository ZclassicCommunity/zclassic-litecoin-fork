    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional ZCL outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes ZCL and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::ZCL_TO_ZCL) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a ZCL-to-ZCL transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::ZCL_ONLY;
        ...
        // If ZCL-to-ZCL fails, create a peg-out transaction
